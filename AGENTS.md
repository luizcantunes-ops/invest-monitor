...
| Alpaca | MCP `~/.Codex/settings.json` | Dados intraday, barras, status do mercado | `config.py` | ✅ Paper account |
...
| Documento | Path | O que cobre |
|-----------|------|-------------|
| SSD — Evolution | `docs/software-design-document.md` | Arquitetura alvo, roadmap de fases, Schwab API, Postgres, deploy |
| Developer Recommendations | `docs/developer-recommendations-from-sdd.md` | Sequência de implementação, P0→P7, harness, hosting |
| Deployment Runbook | `docs/deployment-runbook.md` | Render deploy, env vars, scheduler, rollback, DoD Phase 1 |
| Engineering Recommendations | `docs/engineering-recommendations.md` | Segurança, bugs P0/P1, fontes de dados |
| Security & Data Consistency | `docs/security-and-data-consistency-recommendations.md` | Token interno, atomic writes, brasil helper |

### Roadmap resumido (SSD)
- **Phase 1** — Deploy privado remoto (cloud hosting + auth + token interno) ← **próximo passo**
- **Phase 1.5** — Schwab read-only API (`src/core/schwab.py`) — substituir CSV manual
- **Phase 2** — Postgres como sistema de registro (Supabase ou Neon)
- **Phase 3** — Versão pública anonimizada separada da privada

### Harness de validação
```bash
bash scripts/validate.sh   # 26 checks — sintaxe, secrets, .env, fail-closed, auth, endpoints
```
Rodar antes de todo deploy. Deve retornar `PASS — ready to deploy`.

### Decisões chave (não reverter sem justificativa)
- **Sem trading automático** — `SCHWAB_TRADING_ENABLED=false` como padrão; sem rotas de ordem na fase inicial
- **Databricks não é o runtime de produção** — usar para análise, backtests, lakehouse; não para web hosting
- **Backend exposto direto → 401** — `INTERNAL_API_TOKEN` obrigatório em produção
- **Frontend fail-closed** — `NODE_ENV=production` sem `APP_PASSWORD` → `process.exit(1)`
- **Brasil/cripto — fonte única** — `src/core/brasil.py`; sem duplicatas nem hardcodes
- **Postgres em Phase 2** — Supabase ou Neon; CSV/JSON são fallback/seed, não sistema de registro
